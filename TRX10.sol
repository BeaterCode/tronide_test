/* TRX10 - Tron Chain - investment platform based on Binance Smart Chain blockchain smart-contract technology. Safe and legit!
 *the only official platform of original TRX10 team! All other platforms with the same contract code are FAKE!
*/
...
Contacts
----------------------------------------------
Telegram Chat: https://t.me/trx10platform
...
contract TRX10 {
    ...
    uint256 private constant MIN_WITHDRAW = 0.05 trx;
    uint256 private constant MIN_ADEP = 0.01 trx;
    uint256 private constant MIN_DEP = 0.05 trx;
    uint256 private constant MAX_DEP = 100 trx;
    ...
    function invest(uint256 _referrerCode, uint8 lFl) public payable {
        if (_invest(msg.sender, _referrerCode, lFl, msg.value)) {
            emit onInvest(msg.sender, msg.value);
        }
    }

    function _invest(address _addr, uint256 _referrerCode, uint8 lFl, uint256 _amount) private returns (bool) {
        ...
        _calculateReferrerReward(_amount, lFl, investor.referrer);
    ...
    function withdraw() public {
        ...
        if(withdrawalAmount>0){ 
            ...
            //withdraw
            payable(msg.sender).transfer(withdrawalAmount.sub(reinvestAmount));
    ...
    function _calculateReferrerReward(uint256 _investment, uint8 lFl, uint256 _referrerCode) private {
        ...
        if (_referrerCode != 0) {
            ...
            if (_ref4 != 0) {
                ...
                payable(address(uint160(uid2Investor[_ref4].addr))).transfer(_refAmount);
            }
            ...
            if (_ref5 != 0) {
                ...
                payable(address(uint160(uid2Investor[_ref5].addr))).transfer(_refAmount);
            }
        }
    }
